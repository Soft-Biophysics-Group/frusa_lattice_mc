...
In the `CMakeLists.txt`, after the `model_library` is initialized, we have \
...
Inside the conditional block, the order of the cmake commands will generally be\
the same:
...
## Step 2: Connecting the relevant libraries to `model.cc`
