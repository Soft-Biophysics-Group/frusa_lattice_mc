...
namespace fields_space{
  ...
  void initialize_state(&state_struct state,
    ...
    try{
      if(option=="from_file"){
        std::string input_structure = parameters.input_structure;
        initialize_state_from_file(state,input_structure,parameters);
      }
  ...
  void initialize_interactions(&interactions_struct interactions, 
    ...
    vec1d couplings = parameters.couplings;

    interactions.T_model = couplings[0];
    interactions.coupling_matrix = get_coupling_matrix(couplings);
    interactions.energy = get_energy(interactions.T_model, 
                                     interactions.coupling_matrix);
  }
  /* 
  ...
  double get_energy(double T_model, vec3d coupling_matrix){
  }
