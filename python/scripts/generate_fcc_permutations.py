...
`include/geometry/fcc.h` file.
The function you want to use is `output_formatted_bonds(output_file)`, which puts in a text file
the bond permutations as they will be formatted in the `fcc.h` header.
By default, this script outputs the results in `python/data/geometry/fcc.txt`. To change
...
fcc_geometry = ParticleGeometry.from_lattice_name("fcc")

def format_bond_permutations():
    all_bond_permutations = fcc_geometry.gen_bond_permutations()
    permutations_str = "{\n"
...
def output_formatted_bonds(output_path = cfg.python_path/"data/geometry/fcc.txt"):
