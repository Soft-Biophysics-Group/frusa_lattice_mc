...
void initialize_interactions(state_struct &state,
  ...
  interactions.n_edges = static_cast<int>(std::size(interactions.neighbours));
...
double get_contact_energy(state_struct &state, int site1, int site2,
  ...
  int contact_index{
      get_contact_index(state, site1, site2, interactions)};
...
int get_contact_index_full_full(int state1, int state2, int edge1, int n_states,
  ...
  if (edge1 <= n_edges / 2) {
    ...
    return n_types * n_edges - 1 +
           array_space::hash_3integers(state2, state1,
                                       get_conjugate_edge(edge1), n_states,
                                       n_edges / 2);
  }
}
